...
namespace gh
{
	...
	ExpressionNode* MathematicExpression::parseExpressionConstantOrVariable( const std::string& dataString )
	{
		if( dataString.size() > 0 )
		{
			...
			if( dataChar == '$' )
			{
				...
				{
					std::string tableName = dataString.substr( 1, tableNameIndex - 1 );
					variableNode = new ExpressionNode( "$" + dataString.substr( tableNameIndex + 1 ), tableName );
				}
				else
				{
	...
	int MathematicExpression::evaluateAsInt()
	{
		if( m_rootNode != nullptr )
		{
